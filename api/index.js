...
app.get('/biodiversity:combinations', async (req, res) => {
    ...
    try {
        ...
        switch (GraficaType) {
            case "group":
                var query = 'SELECT "Year","Country", ROUND((avg("CalcBiodivLnd"))::numeric,2) AS "Biodiversity_land",ROUND(((SUM("resultsScen2020"."ProtectedAreasForest" + "resultsScen2020"."ProtectedAreasOtherNat" +"resultsScen2020"."ProtectedAreasOther")) / SUM("resultsScen2020"."TotalLand"))::numeric,2) AS "Protected_land" FROM "resultsScen2020"  WHERE "iteration" = $1 AND "scenathon_id" = $2 GROUP BY "Country","Year" ORDER BY "Country","Year"';
                break;
            case "countries":
                var query = 'SELECT "Year","Country", ROUND((avg("CalcBiodivLnd"))::numeric,2) AS "Biodiversity_land",ROUND(((SUM("resultsScen2020"."ProtectedAreasForest" + "resultsScen2020"."ProtectedAreasOtherNat" +"resultsScen2020"."ProtectedAreasOther")) / SUM("resultsScen2020"."TotalLand"))::numeric,2) AS "Protected_land" FROM "resultsScen2020"  WHERE "iteration" = $1 AND "scenathon_id" = $2 AND "Country" NOT LIKE \'%$_%\' ESCAPE \'$\' GROUP BY "Country","Year" ORDER BY "Country","Year"';
                break;
            case "regions":
                var query = 'SELECT "Year","Country", ROUND((avg("CalcBiodivLnd"))::numeric,2) AS "Biodiversity_land",ROUND(((SUM("resultsScen2020"."ProtectedAreasForest" + "resultsScen2020"."ProtectedAreasOtherNat" +"resultsScen2020"."ProtectedAreasOther")) / SUM("resultsScen2020"."TotalLand"))::numeric,2) AS "Protected_land" FROM "resultsScen2020"  WHERE "iteration" = $1 AND "scenathon_id" = $2 AND "Country" LIKE \'%$_%\' ESCAPE \'$\' GROUP BY "Country","Year" ORDER BY "Country","Year"';
                break;
            default:
                var query = null;
                break;
...
app.get('/forestOne:combinations', async (req, res) => {
    try {
        ...
        switch (GraficaType) {
            case "group":
                var query = 'SELECT "Year", ROUND(sum("NetForestChange")::numeric,2) as "NetForestChange", ROUND(sum(CASE WHEN "NetForestChange" > 0 THEN "NetForestChange" ELSE 0 END)::numeric,2) as "Aforestation", ROUND(sum(CASE WHEN "NetForestChange" < 0 THEN "NetForestChange" ELSE 0 END)::numeric,2) as "ForestLoss", ROUND(sum("gfw_deforestation")::numeric,2) as "GFW_deforestation_global", ROUND(SUM("forest_target")::numeric,2) as "Forest_target" FROM "resultsScen2020" WHERE "iteration"=$1 AND "scenathon_id"=$2 GROUP BY "Year" Order by "Year"';
                break;
            case "countries":
                var query = 'SELECT "Year", ROUND(sum("NetForestChange")::numeric,2) as "NetForestChange", ROUND(sum(CASE WHEN "NetForestChange" > 0 THEN "NetForestChange" ELSE 0 END)::numeric,2) as "Aforestation", ROUND(sum(CASE WHEN "NetForestChange" < 0 THEN "NetForestChange" ELSE 0 END)::numeric,2) as "ForestLoss", ROUND(sum("gfw_deforestation")::numeric,2) as "GFW_deforestation_global", ROUND(SUM("forest_target")::numeric,2) as "Forest_target" FROM "resultsScen2020" WHERE "iteration"=$1 AND "scenathon_id"=$2 AND "Country" NOT LIKE \'%$_%\' ESCAPE \'$\'  GROUP BY "Year" Order by "Year"';
                break;
            case "regions":
                var query = 'SELECT "Year", ROUND(sum("NetForestChange")::numeric,2) as "NetForestChange", ROUND(sum(CASE WHEN "NetForestChange" > 0 THEN "NetForestChange" ELSE 0 END)::numeric,2) as "Aforestation", ROUND(sum(CASE WHEN "NetForestChange" < 0 THEN "NetForestChange" ELSE 0 END)::numeric,2) as "ForestLoss", ROUND(sum("gfw_deforestation")::numeric,2) as "GFW_deforestation_global", ROUND(SUM("forest_target")::numeric,2) as "Forest_target" FROM "resultsScen2020" WHERE "iteration"=$1 AND "scenathon_id"=$2 AND "Country" LIKE \'%$_%\' ESCAPE \'$\'  GROUP BY "Year" Order by "Year"';
                break;
            default:
                var query = null;
                break;
...
app.get('/targets123:combinations', async (req, res) => {
    try {
        ...
        switch (GraficaType) {
            case "group":
                var query = 'SELECT "Year", ROUND(SUM("NetForestChange")::numeric,2) as "NetForestChange", ROUND(SUM("forest_target")::numeric,2) as "Forest_target", ROUND(((SUM("resultsScen2020"."ProtectedAreasForest" + "resultsScen2020"."ProtectedAreasOtherNat" +"resultsScen2020"."ProtectedAreasOther")) / SUM("resultsScen2020"."TotalLand"))::numeric,2) AS "Protected_Land", ROUND(SUM("protected_land_target")::numeric,2) as "Protected_land_target", ROUND((avg("CalcBiodivLnd"))::numeric,2) AS "Biodiversity_Land", ROUND(AVG("BiodivTarget")::numeric,2) AS "Target_Biodiversity" FROM "resultsScen2020" WHERE "iteration" = $1 AND "scenathon_id" = $2 AND "Year" > 2020 GROUP BY "Year" ORDER BY "Year"';
                break;
            case "countries":
    ...
    try {
        ...
        switch (GraficaType) {
            case "group":
                ...
                break;
            case "countries":
                var query = 'SELECT "Year", ROUND((SUM("CalcLiveCH4")/1000)::numeric,2) AS "Livestock_CH4", ROUND((SUM("CalcLiveN2O")/1000)::numeric,2) AS "Livestock_N20", ROUND((SUM("CalcCropN2O")/1000)::numeric,2) AS "Crop_N20", ROUND((SUM("CalcCropCH4")/1000)::numeric,2) AS "Crop_CH4", ROUND((SUM("CalcCropCO2")/1000)::numeric,2) AS "Crop_CO2", ROUND(AVG("ghg_agri_target")::numeric,2) AS "ghg_agri_target", ROUND((AVG("CalcAllLandCO2e")/100)::numeric,2) AS "total_GHG_land", ROUND(AVG("ghg_lu_target")::numeric,2) AS "GHG_LU_target", ROUND(sum("CalcWFblue")::numeric,2) as "BlueWater", ROUND(sum("water_target")::numeric,2) as "water_target" from "resultsScen2020" WHERE "iteration"=$1 and "scenathon_id"=$2 AND "Year"=2050 AND "Country" NOT LIKE \'%$_%\' ESCAPE \'$\' GROUP BY "Year" Order by "Year"';
                break;
            case "regions":
                var query = 'SELECT "Year", ROUND((SUM("CalcLiveCH4")/1000)::numeric,2) AS "Livestock_CH4", ROUND((SUM("CalcLiveN2O")/1000)::numeric,2) AS "Livestock_N20", ROUND((SUM("CalcCropN2O")/1000)::numeric,2) AS "Crop_N20", ROUND((SUM("CalcCropCH4")/1000)::numeric,2) AS "Crop_CH4", ROUND((SUM("CalcCropCO2")/1000)::numeric,2) AS "Crop_CO2", ROUND(AVG("ghg_agri_target")::numeric,2) AS "ghg_agri_target", ROUND((AVG("CalcAllLandCO2e")/100)::numeric,2) AS "total_GHG_land", ROUND(AVG("ghg_lu_target")::numeric,2) AS "GHG_LU_target", ROUND(sum("CalcWFblue")::numeric,2) as "BlueWater", ROUND(sum("water_target")::numeric,2) as "water_target" from "resultsScen2020" WHERE "iteration"=$1 and "scenathon_id"=$2 AND "Year"=2050 AND "Country" LIKE \'%$_%\' ESCAPE \'$\' GROUP BY "Year" Order by "Year"';
                break;
            default:
                var query = null;
                break;
