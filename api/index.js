...
app.get('/targets123:combinations', async (req, res) => {
    try {
        ...
        switch (GraficaType) {
            case "group":
                var query = 'SELECT "Year", ROUND(SUM("NetForestChange")::numeric,2) as "NetForestChange", ROUND(SUM("forest_target")::numeric,2) as "Forest_target", ROUND(((SUM("resultsScen2020"."ProtectedAreasForest" + "resultsScen2020"."ProtectedAreasOtherNat" +"resultsScen2020"."ProtectedAreasOther")) / SUM("resultsScen2020"."TotalLand"))::numeric,2) AS "Protected_Land", ROUND(AVG("protected_land_target")::numeric,2) as "Protected_land_target", ROUND((avg("CalcBiodivLnd"))::numeric,2) AS "Biodiversity_Land", ROUND(AVG("BiodivTarget")::numeric,2) AS "Target_Biodiversity" FROM "resultsScen2020" WHERE "iteration" = $1 AND "scenathon_id" = $2 AND "Year" > 2020 GROUP BY "Year" ORDER BY "Year"';
                break;
            case "countries":
